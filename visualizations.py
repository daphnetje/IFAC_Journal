...
def visualize_performance_across_groups_different_classification_types_with_conf_intervals(data, performance_measure_of_interest, title, y_axis_limits):
    plot = p9.ggplot(data=data, mapping=p9.aes(x="Classification Type", y=performance_measure_of_interest + ' mean', fill="Group")) + \
           ...
           p9.ggtitle(title) + \
           p9.scale_color_manual(values=["#F8766D", "#B79F00", "#00BA38", "#00BFC4", "#2176FF", "#F564E3"])
    return plot
...
def summary_results_to_pretty_chart_plots(dataset, classifier, baseline_acc, baseline_group_fair, baseline_indiv_fair):
    ...
    df = pd.read_excel("final_results\\"+dataset+"\\"+classifier+"multipleIFACS.xlsx") # or pd.read_csv() if needed
...
def summarize_results_per_dataset(dataset, classifier):
    ...
    selective_classifier_names = ["PlugIn", "SCross", "AUC", "Schreuder", "IFAC-GLU", "IFAC_GL", "IFAC_U"]
    ...
    for cov in coverages:
        performance_df = pd.read_excel("final_results\\" + dataset + "\cov = " + cov + ", bb = "+ classifier + "multipleIFACS\\performances.xlsx", engine='openpyxl')
        fairness_df = pd.read_excel("final_results\\" + dataset + "\cov = " + cov + ", bb = "+ classifier + "multipleIFACS\\fairness.xlsx", engine='openpyxl')
    ...
    performance_df.to_excel("final_results\\"+dataset+"\\"+classifier+"multipleIFACS.xlsx")
