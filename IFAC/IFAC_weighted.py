...
class Weighted_IFAC:
    ...
    def fit_discriminatory_associations(self, X):
        ...
        self.val_2_data_with_labels = X_val2_dataset.descriptive_data
        self.val_2_data_with_preds_and_probas = self.make_bb_preds_and_preds_proba_for_data(X_val2_dataset)

        return X_val1_dataset.descriptive_data

    def fit_reject_thresholds(self, w1, w2, w3):
        self.w1 = w1
        self.w2 = w2
        self.w3 = w3
        self.flip_thresholds_per_group, self.reject_threshold_per_group, self.unc_pos_threshold_per_group, self.unc_neg_threshold_per_group, = self.learn_reject_threshold(self.val_2_data_with_labels, self.val_2_data_with_preds_and_probas)
    ...
    def learn_reject_threshold(self, val_data_with_labels, val_data_with_preds):
        ...
        return unf_flip_thresholds_per_intersectional_group, unf_reject_thresholds_per_intersectional_group, \
               unc_pos_threshold_per_intersectional_group, unc_neg_threshold_per_intersectional_group
    ...
    def learn_reject_and_flip_thresholds_unfair_data(self, unfair_part_of_data, slift_per_index, sit_test_scores, n_to_reject, n_to_flip):
        ...
        uncertainty_scores_scaled_for_itemset = (uncertainty_scores_for_itemset) / 0.5

        GLU_scores = (self.w1 * global_unf_scores_for_itemset + self.w2 * local_unf_scores_scaled_for_itemset + self.w3 * uncertainty_scores_scaled_for_itemset) \
    ...
    def predict(self, test_dataset):
        ...
        for pd_itemset in self.intersectional_pd_itemsets:
            ...
            uncertainty_scores_for_itemset = 1 - unfair_part_of_data['pred. probability']
            uncertainty_scaled_scores_for_itemset = (uncertainty_scores_for_itemset) / 0.5

            aggregated_scores = (self.w1 * global_scores_for_itemset + self.w2 * local_scaled_scores_for_itemset + self.w3 * uncertainty_scaled_scores_for_itemset) \
                                + np.random.uniform(-0.01, 0.01, size=len(unfair_part_of_data))
            ...
            to_reject_unfair = aggregated_scores[
                (aggregated_scores < flip_threshold) & (aggregated_scores >= reject_threshold)]
