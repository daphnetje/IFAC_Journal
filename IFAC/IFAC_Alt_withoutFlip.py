...
class IFAC_Alt_NoFlip:

    def __init__(self, coverage, sensitive_attributes, reference_group_list, task, val1_ratio=0.1, val2_ratio=0.1, base_classifier="Random Forest", max_pvalue_slift=0.01, sit_test_k = 10):
    ...
    def fit(self, X):
        ...
        self.reject_threshold_per_group,  self.unc_neg_threshold_per_group, self.unc_pos_threshold_per_group = self.learn_reject_threshold(val_2_data_with_labels, val_2_data_with_preds_and_probas)
        return
    ...
    def learn_reject_threshold(self, val_data_with_labels, val_data_with_preds):
        n_to_reject_per_pd_itemset, n_uncertain_neg_reject_per_pd_itemset, n_uncertain_pos_reject_per_pd_itemset = \
        ...
        for pd_itemset in self.intersectional_pd_itemsets:
            ...
            reject_threshold, non_rejected_data = self.learn_reject_threshold_unfair_data(
                unfair_part_of_data, slift_per_index, sit_test_scores, n_to_reject_per_pd_itemset[pd_itemset])
        ...
        return unf_reject_thresholds_per_intersectional_group, unc_neg_threshold_per_intersectional_group, unc_pos_threshold_per_intersectional_group


    def learn_reject_threshold_unfair_data(self, unfair_part_of_data, slift_per_index, sit_test_scores, n_to_reject):
        ...
        reject_threshold = self.decide_on_reject_threshold_unfair_data(aggregated_disc_scores,
                                                                                       n_to_reject)

        print("Number of unfairness based rejected instances: ", len(aggregated_disc_scores[aggregated_disc_scores >= reject_threshold]))
        ...
        return reject_threshold, non_rejected_data
    ...
    def decide_on_reject_threshold_unfair_data(self, disc_scores, n_instances_to_reject):
        ordered_disc_scores = disc_scores.sort_values(ascending=False)

        if n_instances_to_reject > 0:
            cut_off_reject_score = ordered_disc_scores.iloc[n_instances_to_reject - 1]
        else:
    ...
    def predict(self, test_dataset):
        ...
        for pd_itemset in self.intersectional_pd_itemsets:
            ...
            to_reject_unfair = aggregated_disc_scores[aggregated_disc_scores >= reject_threshold]
        ...
        return predictions, all_unfairness_based_rejects_series, all_uncertainty_based_rejects_series,
    ...
    def calculate_flip_and_reject_rates_per_group(self, data_with_labels, data_with_preds):
        ...
        for itemset in self.intersectional_pd_itemsets:
            ...
            if pos_ratio_for_itemset < target_pos_ratio:
                n_unfair_reject, n_pos_uncertain_rej, n_neg_uncertain_rej = self.calc_n_rejection_for_deprived_group(target_pos_ratio, max_reject, n_itemset, n_pos_for_itemset, pos_ratio_for_itemset)

            else:
                n_unfair_reject, n_pos_uncertain_rej, n_neg_uncertain_rej = self.calc_n_rejection_for_favoured_group(target_pos_ratio, max_reject, n_itemset, n_pos_for_itemset, pos_ratio_for_itemset)
        ...
        return n_unfairness_reject_pd_itemset, n_uncertain_neg_decision_reject_pd_itemset, n_uncertain_pos_decision_reject_pd_itemset
    ...
    def calc_n_rejection_for_deprived_group(self, target_pos_ratio, max_reject, n_itemset, n_pos_for_itemset, pos_ratio_for_itemset):
        ...
        return n_unfair_reject, n_pos_uncertain_rej, n_neg_uncertain_rej
